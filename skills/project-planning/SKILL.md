...
- Get JIRA_PROJECT_KEY from `data/team-context.md` or ask the user## Task Creation (Optional)After the user approves the plan, offer to create tasks in their project tracker.### Jira (if configured)```bash
# Create epic
...
## Fallback Behavior- If Jira is not configured: output plan as markdown. Offer to create GitLab/GitHub issues instead.
