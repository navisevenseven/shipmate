...
```json5
{
  ...
  channels: {
    ...
    telegram: {
      enabled: true,
      botToken: "telegram_token_example",
    ...
    slack: {
      enabled: true,
      botToken: "slack_bot_token_example", // Bot User OAuth Token
      appToken: "slack_app_token_example", // App-Level Token (Socket Mode)
      channels: {
        "*": {
          requireMention: true,
        },
  ...
  env: {
    ANTHROPIC_API_KEY: "anthropic_key_example",
    // or OPENAI_API_KEY: "openai_key_example"
  },
}
```
...
|---------|---------------|-------------|
| Telegram | 1 | `botToken` |
| Slack | 2 | `botToken` (`slack_bot_token_example`), `appToken` (`slack_app_token_example`) |
