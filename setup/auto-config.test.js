...
function setMinimalEnv() {
  ...
  process.env.GITHUB_TOKEN = "github_token_example";
  process.env.TELEGRAM_BOT_TOKEN = "telegram_token_example";
  process.env.ANTHROPIC_API_KEY = "anthropic_key_example";
}
...
describe("buildChannels", () => {
  ...
  it("builds telegram channel from env", () => {
    ...
    assert.equal(ch.botToken, "telegram_token_example");
    assert.equal(ch.dmPolicy, "pairing");
  });

  it("builds slack channel with both tokens", () => {
    process.env.SLACK_BOT_TOKEN = "slack_bot_token_example";
    process.env.SLACK_APP_TOKEN = "slack_app_token_example";
    const { buildSlackChannel } = loadModule();
    const ch = buildSlackChannel();
    assert.equal(ch.botToken, "slack_bot_token_example");
    assert.equal(ch.appToken, "slack_app_token_example");
  });
...
describe("buildModel", () => {
  ...
  it("selects openai when only OPENAI_API_KEY set", () => {
    process.env.OPENAI_API_KEY = "openai_key_example";
    const { buildModel } = loadModule();
    const m = buildModel();
...
describe("generateConfig", () => {
  ...
  it("detects gitlab from GITLAB_TOKEN", () => {
    ...
    process.env.GITLAB_TOKEN = "gitlab_token_example";
    process.env.TELEGRAM_BOT_TOKEN = "telegram_token_example";
    process.env.ANTHROPIC_API_KEY = "anthropic_key_example";
    const { generateConfig } = loadModule();
    const config = generateConfig();
